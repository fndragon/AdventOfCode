...
int main(void)
{
    ...
    for(i=0;i<(1<<size.size());i++) // i's bits from 0 .. size are whether or not a cup is ON.
    {
        ...
        if( sum == 150)
        {
            // figure out the number of bits used for I
            unsigned int numBits = std::popcount(i);
            if (numBits < minimumBitsUsed)
            {
                minimumBitsUsed = numBits;
                count = 1;
            } else if (numBits == minimumBitsUsed) count++; 
            //std::cout << "Found a combo for 150. I is " << i << "\n";
        }
    }
    std::cout << "minimum cups used is " << minimumBitsUsed << " used by " << count << " solutions\n";
    input.close();
    return 0;
}